...
def random_block_gloas(spec, state, signed_blocks, scenario_state, rng=None):
    # Gloas removes execution_payload, so build from altair (not fulu).
    if rng is None:
        rng = Random(3456)
    ...
    # Build a random signed execution payload bid
...
def _build_random_signed_bid(spec, state, block, rng):
    ...
    # Generate random blob commitments
    ...
    if use_real_builder:
        ...
        max_value = builder.balance // 2
        block_hash = spec.Hash32(rng.randbytes(32))
        fee_recipient = builder.execution_address
        value = spec.Gwei(rng.randint(0, max_value)) if max_value > 0 else spec.Gwei(0)
    else:
